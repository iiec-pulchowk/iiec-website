...
from ..database import get_history_db  # Use the history DB session
...
def create_order(order: schemas.OrderHistoryCreate, db: Session = Depends(get_history_db)):
    """
...
def read_orders_history(skip: int = 0, limit: int = 100, db: Session = Depends(get_history_db)):
    """
...
def read_order_history(order_id: int, db: Session = Depends(get_history_db)):
    """
...
def update_order(order_id: int, order_update: schemas.OrderHistoryUpdate, db: Session = Depends(get_history_db)):
    """
    Update an order.
    """
...
def delete_order(order_id: int, db: Session = Depends(get_history_db)):
    """
